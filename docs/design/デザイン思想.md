...
本ドキュメントは、FAQ画面で実装されたモダンなダッシュボードデザインの思想とパターンを定義し、他のページへの横展開を可能にするための設計書です。

**デザインコンセプト**: Google Analytics風のモダンなダッシュボードデザイン  
**準拠規約**: `design.protocol.json` に定義された色・タイポグラフィ・余白・コンポーネント仕様
...
- **実装例**: `frontend/src/views/help/HelpView.vue`
